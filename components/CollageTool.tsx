              ...
              if (lg) {
                ...
                buckets.forEach((arr, av) => {
                  ...
                  /* 半徑倍率：實線 0.50、虛線 0.30。
                     ...
                     虛線用 0.55（比實線再大一點）。線的本體是滿不透明度描在光上面的，
                     等於把「蓋在線身上的那一段光」遮掉，看得見的只剩線周圍那一圈 ——
                     所以半徑太小的話，剩下的光就只是幾顆落在空隙裡的小點，
                     看起來像跟線沒關係（0.30 就是這個樣子）。
                     實測（LINK_W≈3.8）空隙相對虛線的亮度：
                       0.30 → 44%（太緊、變成一顆顆）
                       **0.55 → 60%（每一段周圍都有一圈光，而且還是看得出是虛線）**
                       0.90 → 80%（整條糊成一片，看不出虛線） */
                  const glowK = linkMode === 'dash' ? 0.55 : 0.50;
                  for (const kk of [1, 2, 3]) {
                        ...
                        <div className="space-y-3.5 pt-1 pb-2">
                          ...
                          {!!sel.glow && (
                            <div>
                              ...
                              {swatchStrip(sel.glowColor || sel.color || SHAPE_DEFAULT_COLOR, GLOW_COLORS, (c: string) => patch({ glowColor: c }), true)}
                            </div>
                          )}
                        </div>
                      </div>
