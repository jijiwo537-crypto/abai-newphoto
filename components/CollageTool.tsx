...
export const CollageTool: React.FC<CollageToolProps> = ({ onHome, onRequestExit, initialFile, initialExtras, onImportNew, initialState, histKey, lutList = [] }) => {
  ...
  /* 第一個顏色沒挑過的時候就跟著「遮罩當下的顏色」走（存 null 代表沒挑過），
     第二個固定從純白開始。 */
  const [stripeAPick, setStripeAPick] = useState<string | null>(null);
  const [stripeB, setStripeB] = useState('#FFFFFF');
  ...
  /* ── 發光用的暫存畫布，一定要活在畫面之外 ──────────────────────────
     這兩個原本是宣告在「畫一張」那支函式裡面的區域變數，所以**每畫一格
     就重新 new 一張 canvas**：整層那張是整個預覽那麼大（例如 1571×1350），
  ...
  const addMediaFiles = useCallback((files: File[]) => {
    /* 創意拼圖不收影片（統一走經典拼圖）—— 進來之前先濾掉 */
    const photos = files.filter(f => !isVideoFile(f));
    if (!photos.length) return;
    normalizeImageFiles(photos)
  ...
  const handlePointerDown = (e: React.PointerEvent) => {
    ...
    if (activePointers.current.size === 1) {
      ...
      /* ── 誰在上面就先選誰 ──────────────────────────────────────────
         物件分兩種：沒有標 below 的疊在圖案「之上」，標了 below 的在「之下」。
         以前不管哪一種都比圖案優先，所以物件明明被圖案蓋住，點下去選到的
         還是物件。現在照真正的疊放順序判斷：
           ① 先找在圖案之上的物件
           ② 再看有沒有點到圖案
           ③ 都沒有，才找在圖案之下的物件 */
      if (brushMode === 'off') {
        ...
        const hitObj = (wantBelow: boolean) => {
        ...
        const takeObj = (o: any) => {
          ...
          if (cur0) {
            /* 已經有選中的東西，而這一下按在「別的物件」身上：
               拖 → 移動的仍然是**已選中的那個**（選中之後整個畫布都是它的操作區，
               手指剛好落在別的物件上不該就拖不動）；
               沒拖 → 才把選取換到剛剛按到的這個。 */
            objDragRef.current = {
        ...
        const above = hitObj(false);
        if (above) { takeObj(above); return; }
  ...
  const beginObjStretch = (e: React.PointerEvent, o: any, side: 't' | 'r' | 'b' | 'l', cssK: number) => {
    ...
    objStretchRef.current = { pointerId: e.pointerId, id: o.id, side, startX: e.clientX, startY: e.clientY,
      completeRings: o.kind === 'grid-orbits',
  ...
  const handlePointerUp = (e: React.PointerEvent) => {
    ...
    if (objDragRef.current?.selectOnly && !objDragRef.current.moved) {
      ...
      setSelectedObj(objDragRef.current.pickId ?? objDragRef.current.id);
      setSelectedTarget(null);
    }
    objDragRef.current = null;
    baseDragRef.current = null;
  ...
  const renderToCanvas = useCallback((targetCanvas: HTMLCanvasElement, renderScale: number = 1) => {
    ...
    const holeFrames = new Map<any, HA>();
    const calculateHoleFrame = (h: any): HA => {
    ...
    const hA = (h: any): HA => {
      let frame = holeFrames.get(h);
      if (!frame) { frame = calculateHoleFrame(h); holeFrames.set(h, frame); }
      return frame;
    };
    ...
    /* ── 發光的暫存層：只做「真的有光的那一塊」那麼大 ──────────────────
       box 是這一趟可能有光的範圍（呼叫端算好，座標系跟 paint 一樣）。

       原本這張暫存層跟整個預覽一樣大（例如 1571×1350＝210 萬像素），
       每畫一格都要把它整張清空、整張再貼回目標。實測那一步就吃掉
       **每次 17.9ms**，是發光會卡的唯一原因（相對地，每顆圖案的光暈
       蓋上去總共只花 2.5ms —— 那個早就被快取解決了）。

       現在改成：把範圍換算成畫布上的實際像素，暫存層只開那麼大，
       畫的時候整個座標系往回平移，貼回去時再平移回原位。
       範圍外本來就是全透明、貼上去也不會有任何改變，所以**畫面完全一樣**，
       只是不再每格搬運兩百萬個像素。
       尺寸往上取到 64 的倍數，滑桿在拖的時候才不會每一格都重新配置。 */
    const withGlowLayer = (
      ...
      /* 把範圍換算到畫布的實際像素上。tf 可能有位移與縮放，
         所以拿四個角去換算再取外接矩形，斜的變換也不會算漏。 */
      let rx = 0, ry = 0, rw = W, rh = H;
      if (box) {
        const map = (px: number, py: number) => tf
          ? [tf.a * px + tf.c * py + tf.e, tf.b * px + tf.d * py + tf.f]
          : [px, py];
        const ps = [
        ...
        rx = Math.max(0, Math.floor(Math.min(...xs)) - 2);
        ry = Math.max(0, Math.floor(Math.min(...ys)) - 2);
        rw = Math.min(W, Math.ceil(Math.max(...xs)) + 2) - rx;
        rh = Math.min(H, Math.ceil(Math.max(...ys)) + 2) - ry;
        if (rw <= 0 || rh <= 0) return;
      }
      ...
      // 座標系整個往回平移，畫的內容才會落在這張小畫布的 (0,0) 起
      gg.setTransform(1, 0, 0, 1, -rx, -ry);
      if (tf) gg.transform(tf.a, tf.b, tf.c, tf.d, tf.e, tf.f);
      paint(gg);
    ...
    /* 這張小圖只由「形狀、字、大小、角度、顏色」決定 —— 位置與透明度都不影響它。
       所以照這五項當鍵存起來：同一格裡所有一樣大的圖案共用同一張，
    ...
    const glowBmp = (h: any, sz: number, gcol: string) => {
      ...
      const tg = get2dWide(tmp);
      if (!tg) return null;
      // Render only the shadow, translating the solid caster completely outside
      // the bitmap. No destination-out fringe or second antialiased knockout:
      // those leave a zoom-dependent unlit outline around the visible pattern.
      const casterOffset = side * 2;
      tg.shadowColor = gcol;
      tg.shadowOffsetX = casterOffset;
      for (const kk of [1, 2, 3]) {
        tg.shadowBlur = szQ * 0.045 * 0.5 * kk;
        strokeHoleShape(tg, h, szQ, 0, side / 2 - casterOffset, side / 2, gcol);
      }
    ...
    const glowPass = (
      ...
      /* 這一趟真的可能有光的範圍。
           圖案：半張小圖＝ sz*1.2+4（跟 glowBmp 的 side 同一條式子）
           連線：線寬的 8 倍，實線的三段模糊與虛線的疊層都收在裡面 */
      let bx0 = Infinity, by0 = Infinity, bx1 = -Infinity, by1 = -Infinity;
      const grow = (x: number, y: number, pad: number) => {
        if (x - pad < bx0) bx0 = x - pad;
        if (y - pad < by0) by0 = y - pad;
        if (x + pad > bx1) bx1 = x + pad;
        if (y + pad > by1) by1 = y + pad;
      };
      const halfOf = (sz: number) => (isTextHole(holeType) ? sz * 1.2 : sz * 0.95) + 6;
      items.forEach(it => grow(it.x, it.y, halfOf(it.sz)));
      if (pairs.length) {
        const pad = LINK_W * 8;
        pairs.forEach(([a, b2]) => {
          const pa = hA(a), pb = hA(b2);
          grow(pa.x * s, pa.y * s, pad);
          grow(pb.x * s, pb.y * s, pad);
        });
      }
      ...
      /* ── 快路徑：沒有連線、而且每顆圖案都已經完全不透明 ──────────────
         ...
         滿不透明度畫在同一個位置上 —— 這種情況直接把每顆的光蓋上去就好，
         省掉整層的清空、挖除、以及整塊矩形的合成（那正是會卡的那一步）。
         只要有一顆還在淡入（透明度 < 1）、或是有連線，就照原本的路走。

         實測拖「大小」滑桿（發光開著）：
           全部走暫存層　p90 51ms、25 格掉幀
           走這條快路徑　p90 23.6ms、0 格掉幀（跟發光關掉時一樣順） */
      if (!pairs.length) {
        items.forEach(it => glowInto(g, it.h, it.a, it.sz, it.ang, it.x, it.y, glowColorOf(side)));
        return;
      }
      const done = withGlowLayer(g, g.canvas.width, g.canvas.height,
        gg => {
          items.forEach(it => glowInto(gg, it.h, it.a, it.sz, it.ang, it.x, it.y, glowColorOf(side)));
          if (pairs.length) {
    ...
    if (basePat) {
      ...
      /* 滿版沒有遮罩尺寸（maskW/maskH 都是 0），但圖案仍屬於完整的圖片區。
         不能把 0 當成映射倍率，否則每顆圖案會先被折回遮罩原點，Mobile
         Safari 對零尺寸 pattern 的轉換更可能直接略過填色。滿版明確使用
         圖片自身的 1:1 座標，顏色仍沿用既有遮罩色／自訂遮罩 pattern。 */
      const rx = layout === FULL ? 1 : (iw > 0 ? maskW / iw : 1);
      const ry = layout === FULL ? 1 : (ih > 0 ? maskH / ih : 1);
    ...
    /* ── 置底物件實際蓋到的那一塊（畫布座標，含旋轉後的外接框）───────────
       補畫遮罩圖案那一層本來是「整片有洞的區域」那麼大，可能好幾百萬像素，
       每一格都要整層貼一次 —— 那就是「物件沉到圖案底下之後拖起來很卡」。
       其實只有「被物件蓋住的那一小塊」需要補畫，所以把那一層縮到這個範圍。 */
    const belowBox = (() => {
      if (!belowObjs.length) return null;
      let x0 = Infinity, y0 = Infinity, x1 = -Infinity, y1 = -Infinity;
      belowObjs.forEach(o => {
        const { bw: ow, bh: oh } = aabbOf(o.w || 0, o.h || 0, o.rot || 0);
        const cx = (o.x + (o.w || 0) / 2) * s, cy = (o.y + (o.h || 0) / 2) * s;
        // 描邊與發光會長到框外面，多留一點餘裕
        const pad = Math.max(ow, oh) * s * 0.25 + 24 * s;
        x0 = Math.min(x0, cx - ow * s / 2 - pad); y0 = Math.min(y0, cy - oh * s / 2 - pad);
        x1 = Math.max(x1, cx + ow * s / 2 + pad); y1 = Math.max(y1, cy + oh * s / 2 + pad);
      });
    ...
    const drawMaskHolesOnTop = () => {
      ...
      /* 只補畫「置底物件真的蓋到」的那一塊 —— 其他地方本來就沒被蓋住，
         補畫過去畫出來的跟原本一模一樣，純粹是白花時間（見 belowBox）。 */
      if (belowBox) {
        tx0 = Math.max(tx0, belowBox.x0); ty0 = Math.max(ty0, belowBox.y0);
        tx1 = Math.min(tx1, belowBox.x1); ty1 = Math.min(ty1, belowBox.y1);
        if (!(tx1 > tx0 && ty1 > ty0)) return;   // 完全沒有交集
      }
    ...
    if (isMain && !hideChromeRef.current && groupFlashProgress >= 0 && groupFlashProgress < 1) {
      ...
      const frameOne = (h: any, x: number, y: number) => {
        ...
        if (isTextHole(holeType)) {
          ...
          const box = patternPathBounds(holeType, sz), pad = 4 * uiPx;
  ...
  /* ── 影片自己的那支迴圈 ──────────────────────────────────────────────
     ...
     幾個刻意的決定：
       ...
       ‧ **影格沒有換就一個字都不畫。** 素材是 24／25／30fps，畫布卻照 30fps
         盲目重畫的話，每五格就有一格是完全一樣的畫面 —— 白燒的電。
       ‧ **播放中改用「一個畫布像素對一個裝置像素」。** 靜止畫面用 1.8 倍
  ...
  useEffect(() => {
    const onVis = () => {
      ...
      if (document.hidden) pauseVideos(list);
        ...
        {imageState && selectedObj && !objDragging && !composeState && !tuningEdge && !objPinching && (() => {
          ...
          /* ── 這排鍵要貼著「畫面上真的看得到的那個選取框」 ──────────────────
             ...
             但畫出來的框有兩件事會讓它跟原始方框不一樣：
               ① 選了外形（圓形／星星／愛心）或裁切過，框會縮到剛好包住實際的圖
                  （imgShapeInk）—— 愛心上面那片空白不算在內。
               ② 物件轉過角度時框跟著轉，最低的點是轉完之後的那個角。
             兩者都會讓這排鍵離框好一段，看起來就是「它還停在原本的位置」。
