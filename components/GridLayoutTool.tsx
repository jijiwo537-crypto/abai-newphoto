  ...
  4: [
    ...
    },
    {
      ...
      rects: [
        { x: 0, y: 0, w: 1, h: .5 },
        { x: 0, y: .5, w: 1, h: .5 },
        { x: .34, y: .09, w: .32, h: .32, squareOverlay: true },
        { x: .34, y: .59, w: .32, h: .32, squareOverlay: true }
      ]
    }
  ],
  5: [
      ...
      paint: (ctx, density) => {
        ...
        if (isScenePhoto) {
          ...
          if (source.complete && source.naturalWidth) {
            ...
            // A fraction of one source texel is numerical tolerance, not a new
...
export const GridLayoutTool: React.FC<GridLayoutToolProps> = ({ histKey, onHome, onRequestExit, onImportNew, initialFiles, initialState, lutList = [] }) => {
  ...
  const applySnapping = (
    ...
    /* 圖形吸附必須用「真正畫到的墨水」，不能拿儲存用的 width/height 外盒。
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      // Pinch already commits geometry and ink together in its touch handler.
      // An idle transition loop must not repeat that work twice each frame.
      if (!anim) { raf = requestAnimationFrame(tick); return; }
      if (anim) {
  ...
  /* .safe-top 會建立 fixed containing block；Touch.clientX/Y 卻是 viewport 座標。
     直接寫入 clientY 會把 iPhone 狀態列高度再加一次，縮圖便落在手指下方。 */
  const dragThumbPoint = (clientX: number, clientY: number) => {
  ...
  const handleCellTouchMove = (e: React.TouchEvent<HTMLDivElement>, idx: number, layoutId: string) => {
    ...
    if (touchZoomState.current && e.touches.length >= 2) {
      ...
      if (touchDragState.current) {
        ...
        if (floatingEl) {
          const p = dragThumbPoint(touch.clientX, touch.clientY);
          floatingEl.style.transform = `translate3d(${p.x}px, ${p.y}px, 0) translate(-50%, -50%) scale(1.15) rotate(4deg)`;
        }
  ...
  const handleReplaceFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const files = Array.from(e.target.files || []).slice(0, targetLayout.images.length);
    if (files.length === 0) return;
    ...
    const loadedFiles = (await Promise.allSettled(loadPromises))
      .flatMap(result => result.status === 'fulfilled' ? [result.value] : []);

    const destinations = files.length > 1
      ? targetLayout.images.map((_, index) => index)
      : [targetIdx ?? targetLayout.images.findIndex(cell => !cell.url)].filter(index => index >= 0);
    setPages(previous => previous.map(page => ({...page, layouts: page.layouts.map(layout => {
      if (layout.id !== targetLayout.id) return layout;
      const updated = [...layout.images];
      loadedFiles.forEach(({file, url, dims}, index) => {
        const slot = destinations[index];
        if (slot === undefined || !updated[slot]) return;
        updated[slot] = {...updated[slot], file, url, zoom: 1, offsetX: 0, offsetY: 0, rotation: 0,
          naturalWidth: dims.width, naturalHeight: dims.height};
      });
      return {...layout, images: updated};
    })})));
    /* Batch import commits to its original layout even if the selection changes
       while photographs are decoding. */
  ...
  const handleFloatSwapTouchMove = (e: React.TouchEvent) => {
    ...
    if (s.dragging) {
      ...
      if (el) {
        const p = dragThumbPoint(t.clientX, t.clientY);
        el.style.transform = `translate3d(${p.x}px, ${p.y}px, 0) translate(-50%, -50%) scale(1.1) rotate(4deg)`;
      }
  ...
  /**
   * 第一根手指按下時就鎖定本次手勢操作的物件。iOS 在第二根手指落下時，
   * React 送來的 target 可能會變成透明控制層或頁面本身；若此時才依 target
  ...
  const handleWorkspaceTouchStart = (e: React.TouchEvent<HTMLDivElement>) => {
    ...
    if (kind) {
      ...
      const gestureCellIdx = kind === 'cell'
        ? (lockedTarget?.kind === 'cell' ? (lockedTarget.cellIdx ?? -1) : (selectedIndex ?? -1))
        : -1;
      const cell = kind === 'cell' && gestureCellIdx >= 0 ? images[gestureCellIdx] : undefined;
      ...
      if (g.mode === 'pinch' && twoFinger) {
        ...
        if (g.kind === 'floating') {
          ...
          if (selectedImg) {
            ...
            /* 圖片碰到圖形時只是不觸發任何交換／套用效果，不能把位置鎖在
               上一幀。舊的碰撞阻擋會讓圖片一碰到圖形便像撞牆卡死；現在圖片
               始終跟隨手指穿過，圖形本身完全不回應。 */
            queueInteraction(() => {
              setActiveGuidelines(guidelines);
              setFloatingImages(prev => prev.map(img =>
    ...
    try {
      ...
      const drawPageLayout = async (ctx: CanvasRenderingContext2D, pageIdx: number, layout: LayoutItem) => {
        const pageOffsetX = pageIdx * targetW;
        {
          ...
          pageActiveTemplate.rects.forEach((rawRect, idx) => {
  ...
  return (
    <div ref={gridRootRef} className="safe-top flex flex-col w-full h-screen bg-black text-white relative font-sans overflow-hidden">
                      ...
                      {finalKinds[i] === 'video' ? (
                        ...
                        <video
                          ...
                          /* 匯出結果只是「看成品」，不該有播放器那一套控制項
                             （暫停、快轉、音量、AirPlay…）。拿掉 controls＝單純自動輪播；
                             再擋掉子母畫面與長按跳出的原生選單。 */
      ...
      <div
        ...
        /* IG 淡入時不能透出底下的原預覽圖片；visibility 不會改變任何幾何，
           因此關閉後畫布直接就在原位。 */
        style={{ visibility: igPreview ? 'hidden' : undefined }}
        onPointerDown={(e) => {
          if (activeTab === 'motion') return;
      ...
      >
        ...
        <div 
          ...
          onScroll={(e) => {
            ...
            positionPageCtls();
            /* 整體 pinch 時 scrollLeft 每一幀都會改。這時更新 activePageIndex
                          ...
                          >
                            ...
                            {page.layouts.map((layout) => {
                              ...
                              {(() => {
                                ...
                                return pageActiveTemplate.rects.map((rawRect, idx) => {
                                  const cell = layout.images[idx];
                                  ...
                                  return (
                                    <div
                                      ...
                                      style={{
                                        ...
                                        padding: `${gap / 2}px`,
                                        transition: layoutTransition,
                                        zIndex: rect.squareOverlay ? 10 : (isThisLayoutSelected && (isDragOver || touchDragOverIndex === idx || hoveredSwapTargetIndex === idx))
                                          ? 4
